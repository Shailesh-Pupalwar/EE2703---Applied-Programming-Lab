...
import scipy.integrate as integral # To integrate
...
C_Exp[0][0] = (1/(2*π))*(integral.quad(Exp, 0, 2*π))[0] # Put x = 0, k = 0; to get a0 (Constant Fourier coefficient)        
for i in range(1, 26):                                  # i varies from 1 to 25
    C_Exp[(2*i)-1][0] = (1/π)*(integral.quad(u_Exp, 0, 2*π, args=(i)))[0]  # Calculating 'an' Fourier coefficients and assigning it to odd terms of matrix(C[1], C[3]..)
    C_Exp[2*i][0] = (1/π)*(integral.quad(v_Exp, 0, 2*π, args=(i)))[0]   # Calculating 'bn' Fourier coefficients and assigning it to even terms of matrix(C[2], C[4]..)

C_coscos = np.zeros((51,1))                             # Initialising C_Exp array to store Fourier coefficients of function exp(x)
C_coscos[0][0] = (1/(2*π))*(integral.quad(coscos, 0, 2*π))[0] # Put x = 0, k = 0; to get a0 (Constant Fourier coefficient)  
for i in range(1,26):                                   # i varies from 1 to 25                                 
    C_coscos[(2*i)-1][0] = (1/π)*(integral.quad(u_coscos, 0, 2*π, args=(i)))[0] # Calculating 'an' Fourier coefficients and assigning it to odd terms of matrix(C[1], C[3]..)
    C_coscos[2*i][0] = (1/π)*(integral.quad(v_coscos, 0, 2*π, args=(i)))[0] # Calculating 'bn' Fourier coefficients and assigning it to even terms of matrix(C[2], C[4]..)
...
#semilogy(x, Exp(t), 'g', label = 'Periodic Extension') # Plotting semilog for periodic extension of exp(x) function
#semilogy(xt, Est_C_Exp, 'bo', label  = 'Estimated Value') # Plotting semilog for estimated exp(x) function
...
show() # Dispalying all the graphs above
